    ...
    <div v-if="isDataLoaded">
      ...
      <div
        class="
          ...
          p-8 rounded-md shadow-md 
          bg-light-grey relative"
      >
        <div 
          v-if="user" 
          class="flex absolute left-2 top-2 gap-x-2"
        >
          <div
            class="
              h-7 w-7 
        ...
        <span
          class="
            mt-6 py-1.5 px-5 
            text-xs text-white bg-at-light-green
        ...
        <div class="w-full mt-6">
          <input
            ...
            v-model="data.trainingName"
          />
          <h1 
            v-else 
            class="text-at-light-green text-2xl text-center">
            {{ data.trainingName }}
          </h1>
        </div>
      </div>
      ...
      >
        <!-- Strength Training -->
        <div 
          v-if="data.trainingType === 'strength'" 
          class="flex flex-col gap-y-4 w-full"
        >
          <div
            class="
              flex flex-col gap-x-6 gap-y-2 
              relative 
              sm:flex-row"
...
export default {
  ...
  setup() {
    ...
    const currentId = route.params.trainingId
    const getData = async () => {
      try {
    ...
    const addExercise = () => {
      if(data.value.trainingType === 'strength') {
    ...
    const update = async () => {
      try {
        const { error } = await supabase
          .from('trainings')
          .update({
            trainingName: data.value.trainingName,
            exercises: data.value.exercises
          })
          .eq('id', currentId)
        if(error) throw error
        editMode()
        statusMsg.value = "Success: training updated!"
        setTimeout(() => {
          statusMsg.value = ""
        }, 5000);
      } catch (error) {
